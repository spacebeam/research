...
local IMAGE_FILES = {
	...
	"light_player", "light_fire", "light_fireball", "shockwave",
...
local UNIT_SPRITES = {
	...
	"magmahulk_rage_jump", "magmahulk_rage_land", "magmahulk_portrait",
	"human_1_run", "human_2_run", "human_3_run", "human_4_run", "human_5_run",
	"human_1_carry_left", "human_2_carry_left", "human_3_carry_left", "human_4_carry_left", "human_5_carry_left", 
	"human_1_carry_right", "human_2_carry_right", "human_3_carry_right", "human_4_carry_right", "human_5_carry_right",
	"human_1_fly", "human_2_fly", "human_3_fly", "human_4_fly", "human_5_fly",
	"human_1_burn", "human_2_burn", "human_3_burn", "human_4_burn", "human_5_burn",
	"human_1_panic", "human_2_panic", "human_3_panic", "human_4_panic", "human_5_panic"
}

local BACKGROUND_FILES = { "night" }

local SOUND_FILES = { "blip" }

NUM_ROOMS = { [10] = 6, [11] = 6, [17] = 6, [24] = 6 }

NO_CASUALTIES_MESSAGES = {
	"REMEMBER: YOUR JOB IS TO RESCUE THE COMPUTER AI BOTS.\nNOT TO PUT OUT FIRE!",
	"KEEP UP THE GOOD WORK, BUDDY!\nYOU'RE ON FIRE.\nHE HE HE",
	"REMEMBER TO SCOUT FOR VALUABLE POWERUPS.\nTHEY WILL COME IN HANDY LATER",
	"DON'T WANT TO WASTE WATER ON OPENING DOORS?\nTRY THROWING BOTS AT THEM",
	"RESCUING 3 OR MORE BOTS IN SHORT SUCCESSION GIVE YOU A SMALL BONUS",
	"REMEMBER: COLLECTING STUFF IS ESSENTIAL FOR YOUR SURVIVAL"
}

WON_MESSAGES = {
	{"CONGRATULATIONS!","","YOU HAVE BEATEN MR. MAGMA","AND RESCUED THE SMALL BUSINESS",
	 "","NOW TRY TO RESCUE","THE LAB COMPLEX!","","PRESS RETURN TO CONTINUE"},
	{"CONGRATULATIONS!","","YOU HAVE BEATEN MR. LEAK","AND RESCUED THE LAB COMPLEX",
	 "","NOW TRY TO RESCUE","THE BIG CORPORATION!","","PRESS RETURN TO CONTINUE"},
	{"CONGRATULATIONS!","","YOU HAVE BEATEN MR. COAL","AND RESCUED THE BIG CORPORATION",
	 "","YOU ARE NOW TRUE","MR. BOT RESCUE STAR IN HEART!","","PRESS RETURN TO CONTINUE"}
}
...
BUILDING_NAMES = {{"SMALL","BUSINESS"},{"LAB","COMPLEX"},{"BIG","CORPORATION"}}
DIFFICULTY_NAMES = {"PIZZERIA", "ROBOTICS", "POLITICS"}
...
function loadResources()
	...
	for i,v in ipairs(UNIT_SPRITES) do
		img[v] = lg.newImage("data/units/"..v..".png")
	end
	...
	for i,v in ipairs(SOUND_FILES) do
		snd[v] = love.audio.newSource("data/sounds/"..v..".wav","static")
		snd[v]:addTags("sfx")
	end

	img.human_run = { img.human_1_run, img.human_2_run, img.human_3_run, img.human_4_run, img.human_5_run }
	img.human_carry_left = { img.human_1_carry_left, img.human_2_carry_left, img.human_3_carry_left, img.human_4_carry_left, img.human_5_carry_left }
	img.human_carry_right = { img.human_1_carry_right, img.human_2_carry_right, img.human_3_carry_right, img.human_4_carry_right, img.human_5_carry_right }
	img.human_fly = { img.human_1_fly, img.human_2_fly, img.human_3_fly, img.human_4_fly, img.human_5_fly }
	img.human_burn = { img.human_1_burn, img.human_2_burn, img.human_3_burn, img.human_4_burn, img.human_5_burn }
	img.human_panic = { img.human_1_panic, img.human_2_panic, img.human_3_panic, img.human_4_panic, img.human_5_panic }
...
function playMusic(name, loop)
	...
	-- Play new file
	music_name = name
	music = love.audio.newSource("data/sounds/"..name..".ogg", "stream")
	music:addTags("music")
	if loop ~= nil then
